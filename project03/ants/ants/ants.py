...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if origin_ant.can_contain(self) == False and self.can_contain(origin_ant) == False:
                assert False, 'Too many ants in {0}'.format(place)
            if origin_ant.can_contain(self):
                origin_ant.store_ant(self)
            else:
                place.ant = self
                self.store_ant(origin_ant)
            # END Problem 8b
    ...
    def double(self):
        ...
        if self.is_container == True and self.ant_contained != None:
            contained_ant = self.ant_contained
            if contained_ant.is_doubled == False:
                contained_ant.is_doubled = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while (current_place != None and current_place.bees == [] and current_place.is_hive == False) or count < self.lower_bound:
            if current_place == None:
                break
            current_place = current_place.entrance
            count += 1
        if current_place == None or current_place.is_hive == True or current_place.bees == [] or count > self.upper_bound:
            return None
        else: return random_bee(current_place.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        flag = amount >= self.health
        reflected = amount + self.damage * flag
        place = self.place
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.can_contain(ant) == False:
            assert False, "{} cannot contain {}".format(self, ant)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place != None:
            if current_place.ant != None and current_place is not self.place:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if current_place.ant.is_container == True and current_place.ant.ant_contained != None:
            dict[current_place.ant] = distance
        for bee in current_place.bees:
                dict[bee] = distance
        current_place, distance = current_place.entrance, distance + 1
        while current_place != None and current_place.is_hive == False:
            if current_place.ant != None :
                dict[current_place.ant] = distance
                if current_place.ant.is_container == True and current_place.ant.ant_contained != None:
                    dict[current_place.ant.ant_contained] = distance
            for bee in current_place.bees:
                dict[bee] = distance
            current_place, distance = current_place.entrance, distance + 1
        return dict
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.damage - distance * 0.25
